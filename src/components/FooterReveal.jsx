...
const HudLink = ({ href, label, delayOffset }) => {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.9, color: "#ffffff" }} // MOBILE FIX: Shrinks and brightens when tapped
    ...
    <section ref={footerRef} className="h-screen bg-black text-white px-6 md:px-20 flex flex-col justify-center items-center border-t border-zinc-900 z-10 relative overflow-hidden py-10">
      ...
      >
        ...
        <motion.div variants={textItemVariants} className="mb-16 md:mb-20">
          {/* Change <button to <motion.button and add whileTap */}
          <motion.button 
            onClick={onInitiateComms}
            whileTap={{ scale: 0.95 }}
            className="relative group inline-flex items-center justify-center px-8 py-4 md:px-12 md:py-5 bg-white text-black font-black uppercase tracking-widest text-sm md:text-base overflow-hidden cursor-pointer outline-none focus:outline-none"
          >
        ...
        <div className="w-full flex flex-row flex-wrap justify-center items-center gap-6 md:gap-16 relative">
          ...
          {/* NEW: CV Link in Footer */}
          <HudLink href="https://1drv.ms/b/c/907395d00dce913c/IQC0s8ZZMOUeTYBKZqEHomwLATXLRwxc_LIMvX1O4-ZVH7Q?e=TwZ2MB" label="RESUME" delayOffset={1.0} />
        </div>
      </motion.div>
    </section>
  );
