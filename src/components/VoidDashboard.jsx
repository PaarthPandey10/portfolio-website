...
const ThorGrid = ({ children, isMobile }) => {
...
const ThorSkill = ({ children, healSignal, isMobile }) => {
...
const InteractiveListItem = ({ children, healSignal }) => {
  ...
  return (
    <motion.div 
      ...
      className={`flex gap-4 items-start cursor-pointer md:cursor-grab active:cursor-grabbing group p-2 -ml-2 rounded-md hover:bg-zinc-900 active:bg-zinc-900 transition-colors text-zinc-400 ${isNeutralized ? 'line-through text-zinc-800 opacity-50' : ''}`}
    >
      <motion.span 
  ...
  return (
    ...
    >
      <ReactLenis root={false} className="h-full overflow-y-auto no-scrollbar p-6 md:p-16">
        ...
        >
          <motion.span transition={{ duration: 0.3, ease: "easeOut" }} className="inline-block text-white group-hover:-translate-x-2 transition-transform">{'<<'}</motion.span> 
          REVERT TO CORE_
        </motion.button>
        ...
        <motion.div variants={containerVariants} initial="initial" animate="animate" className="max-w-7xl mx-auto w-full">
          <div className="mb-16 border-b border-zinc-800 pb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-8" style={{ perspective: 1000 }}>
            <div className="flex-1">
              <motion.div onClick={handleKingHeal} whileHover={{ scale: 1.01 }} animate={isHealing ? { scale: 1.05, y: -15, rotateX: 15, textShadow: "0px 10px 50px rgba(255,255,255,0.9), 0px 0px 80px rgba(255,255,255,0.5)" } : { scale: 1, y: 0, rotateX: 0, textShadow: "none" }} transition={{ type: "spring", stiffness: 100, damping: 10 }} className="cursor-pointer inline-block">
                <BlurText text={data.title || "ROLE_TITLE"} delay={30} animateBy="words" direction="top" className="text-[10vw] md:text-[6vw] font-black uppercase tracking-tighter leading-none mb-6" />
              </motion.div>
              <motion.div variants={itemVariants} className="flex flex-wrap gap-4 font-mono text-xs md:text-sm text-zinc-500 tracking-widest uppercase items-center">
                {metaItems.map((item, idx) => (
                  <React.Fragment key={idx}>
                    <motion.button onClick={() => handleWhack(idx)} animate={whackedIndices.includes(idx) ? { y: 6, scaleY: 0.15, scaleX: 1.1, opacity: 0.4 } : { y: 0, scaleY: 1, scaleX: 1, opacity: 1 }} whileHover={!whackedIndices.includes(idx) ? { y: -4, scale: 1.05, color: "#ffffff" } : {}} transition={{ type: "spring", stiffness: 500, damping: 15 }} className="px-3 py-1 bg-zinc-900 border-b-2 border-zinc-700 hover:bg-zinc-800 rounded-sm cursor-crosshair select-none">{item}</motion.button>
              ...
              <div className="bg-black p-8 md:p-12 pointer-events-auto min-h-[400px]">
                ...
                <Reorder.Group axis="y" values={responsibilities} onReorder={setResponsibilities} className="space-y-2">
                  {responsibilities.map((res) => (
                    <Reorder.Item key={res} value={res} dragListener={!isMobile}>
              ...
              <div className="bg-black p-8 md:p-12 pointer-events-auto min-h-[400px]">
                ...
                <ul className="flex flex-wrap gap-4">
                  {data.skills?.map((skill, idx) => (
                    <ThorSkill key={idx} healSignal={healSignal} isMobile={isMobile}>{skill}</ThorSkill>
                  ))}
                </ul>
              </div>
            </div>
